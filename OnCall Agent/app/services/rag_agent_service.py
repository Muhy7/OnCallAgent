...
使用标准 LangChain OpenAI 集成，兼容 Python 3.10，调用阿里云 DashScope。
"""
...
# 阿里云千问大模型兼容 OpenAI 接口
# 参考：https://help.aliyun.com/zh/dashscope/developer-reference/compatibility-with-openai
...
class RagAgentService:
    """RAG Agent 服务 - 使用 LangGraph + OpenAI 兼容模式（适配阿里云 DashScope）"""
    ...
    def __init__(self, streaming: bool = True):
        ...
        # 使用标准 ChatOpenAI 兼容模式连接阿里云 DashScope
        ...
        logger.info(f"RAG Agent 服务初始化完成 (OpenAI兼容模式), model={self.model_name}, streaming={streaming}")
            ...
            async for token, metadata in self.agent.astream(
                ...
                if message_type in ("AIMessage", "AIMessageChunk"):
                    ...
                    if content_blocks and isinstance(content_blocks, list):
                        ...
                        # 标准的文本内容直接从 .content 属性取
                        if hasattr(token, 'content') and token.content:
                            text_content = token.content if isinstance(token.content, str) else str(token.content)
                            if text_content:
                                yield {
                                    "type": "content",
                                    "data": text_content,
