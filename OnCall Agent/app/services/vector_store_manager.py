"""向量存储管理器 - 封装 Milvus VectorStore 操作（完全懒加载模式）"""
...
class VectorStoreManager:
    ...
    def __init__(self):
        """初始化向量存储管理器（不立即连接 Milvus）"""
        self._vector_store: Optional[Milvus] = None
        self.collection_name = COLLECTION_NAME
        logger.info("VectorStoreManager 已创建（延迟初始化模式）")

    def _initialize_vector_store(self) -> Milvus:
        """延迟初始化 Milvus VectorStore（首次调用时才执行）"""
        ...
        try:
            ...
            raise RuntimeError(f"向量存储功能未就绪: {e}") from e
